...
let s:is_running_win = has('win16') || has('win32') || has('win64')

let s:is_running_linux = has('linux')
let s:is_running_unix = has('unix')
let s:is_running_macunix = has('mac') || has("macunix")
let s:is_running_cygwin = has('win32unix')

let s:is_running_gui = has("gui_running")
let s:is_running_gui_macvim = has("gui_macvim")
"}
...
if !filereadable(neobundle_readme)
    ...
    execute 'silent !git clone https://github.com/Shougo/neobundle.vim.git "' . expand('$VIMBUNDLE/neobundle.vim') . '"'
...
" --------------------------------------------------------------------
    ...
    if count(g:billinux_bundle_groups, 'snipmate')
        ...
        if filereadable(expand($VIMBUNDLE)."/vim-snippets/snippets/support_functions.vim")
...
if s:is_running_gui
    ...
    if s:is_running_linux || s:is_running_unix || s:is_running_cygwin
        set guifont=Source\ Code\ Pro\ for\ Powerline:12,DejaVu\ Sans\ Mono\ for\ Powerline:h11,
    elseif s:is_running_macunix
        set guifont=Source\ Code\ Pro\ for\ Powerline:12,Meslo\ LG\ for\ Powerline:h11,Menlo\ Regular\ for\ Powerline:h15
    elseif s:is_running_win
        set guifont=Source\ Code\ Pro\ for\ Powerline:12,Bistream\ Vera\ Sans\ Mono\ for\ Powerline:h11,DejaVu\ Sans\ Mono\ for\ Powerline:h11,
    endif
...
if isdirectory(expand($VIMBUNDLE) . "/nerdtree")
    ...
    imap <leader>t :NERDTreeToggle<cr>
    nmap <leader>t :NERDTreeToggle<cr>
    imap <leader>T :NERDTreeFind<cr>
...
if has('unix')
    ...
    if isdirectory(expand($VIMBUNDLE) . "/vim-clangcomplete")
...
if has('gui_running')
    if filereadable(expand($HOME)."/.gvimrc.local")
        source expand($HOME)."/.gvimrc.local"
    endif
endif
"}
...
" On Windows, the $HOME environment variable in Vim is set to %HOMEDRIVE%%HOMEPATH%
"g:  global variables
